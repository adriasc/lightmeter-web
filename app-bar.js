...
const GRID_ROWS = 10;
const GRID_COLS = 14;
// Two measured points map the auto-exposed preview signal back to scene EV100.
const PREVIEW_SIGNAL_LOW = 4.3;
const SCENE_EV100_LOW = 3.65;
const PREVIEW_SIGNAL_HIGH = 5.4;
const SCENE_EV100_HIGH = 8.0;
...
const METER_CALIBRATION_STORAGE_KEY = "filmLightMeterCalibrationStopsV4";
...
function calculateSceneEV100(refLuma, frameAverageLuma) {
  ...
  const previewSignal = Math.log2(Math.max(refLuma, 1e-4) * 100);
  return mapPreviewSignalToSceneEV100(previewSignal) + meterCalibrationStops;
}

function mapPreviewSignalToSceneEV100(previewSignal) {
  const previewRange = PREVIEW_SIGNAL_HIGH - PREVIEW_SIGNAL_LOW;
  const sceneRange = SCENE_EV100_HIGH - SCENE_EV100_LOW;
  const normalizedSignal = (previewSignal - PREVIEW_SIGNAL_LOW) / previewRange;
  return clamp(SCENE_EV100_LOW + normalizedSignal * sceneRange, -8, 20);
}
